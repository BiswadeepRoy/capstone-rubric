...
const getCountry = async(city) => {
    const response = await fetch(`http://api.geonames.org/searchJSON?q=${city}&maxRows=1&username=biswa`);
    try {
        const responseData = await response.json();
        const countryID = responseData.geonames[0].countryCode;
        return countryID;
...
const getWeather = async(city = '', country = '', days) => {
    const response = await fetch(`https://api.weatherbit.io/v2.0/forecast/daily?city=${city},${country}&key=6bac312881784f5695dc61d05add4fa9&units=M&days=${days}`);
    try {
        const responseData = await response.json();
